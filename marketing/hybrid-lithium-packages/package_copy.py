...
TIER_CUSTOMER_NOTES: dict[str, str] = {
    "ep-6.5kva": (
        "6.5 KVA tier with a matching 6.5 kW inverter. Eight panels (4.6 kWp) are sized for this load — "
    ...
    "ep-8kva": (
        ...
        "still plan within ~6,800 W continuous. Ten panels (5.7 kWp) match the 8 KVA tier, not 10 kW maximum."
    ),
    "ep-10kva": (
        "10 KVA tier with one 10 kW inverter — aligned. Fifteen panels (8.6 kWp). "
        "Run up to ~8,500 W continuous; do not run two split AC units on full cool at the same time."
    ),
    "ep-12kva": (
        "12 KVA tier (~10,200 W planned use). Two 6.5 kW inverters work together (~13 kW available). "
        "Eighteen panels (10.3 kWp) are sized for 12 KVA load, not 13 kW inverter maximum."
    ),
    "ep-15kva": (
        "15 KVA tier (~12,800 W planned use). Two 10 kW inverters (synchronized, stocked) handle peaks; "
        "twenty-four panels (13.7 kWp) are sized for 15 KVA — less solar than the 20 KVA tier on the same inverter pair."
    ),
    "ep-20kva": (
        "20 KVA tier (~17,000 W planned use) with two 10 kW inverters. "
        "Thirty-six panels (19.3 kWp) match this load; guest-house and hotel loads need a written load schedule on survey."
    ),
}
...
TIER_BROCHURE_NOTES: dict[str, str] = {
    "ep-6.5kva": "6.5 KVA tier · matching 6.5 kW inverter · 8 panels (4.6 kWp) · no AC in this tier.",
    "ep-8kva": "8 KVA (~6,800 W). 10 kW inverter for AC starts · 10 panels (5.7 kWp) for 8 KVA load.",
    "ep-10kva": "10 KVA / 10 kW aligned · 15 panels (8.6 kWp) · stagger two AC units.",
    "ep-12kva": "12 KVA (~10,200 W) · dual 6.5 kW (~13 kW) · 18 panels (10.3 kWp) for 12 KVA load.",
    "ep-15kva": "15 KVA (~12,800 W) · dual 10 kW sync · 24 panels (13.7 kWp).",
    "ep-20kva": "20 KVA (~17,000 W) · dual 10 kW · 36 panels (19.3 kWp) · load schedule on survey.",
}
