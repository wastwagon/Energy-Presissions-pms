#!/usr/bin/env python3
"""
Run Alembic migrations and seed default settings / peak sun hours on a target Postgres.

Requires PROD_DATABASE_URL (full postgresql:// URL). Never commit database passwords.

Example:
  export PROD_DATABASE_URL='postgresql://user:pass@host.region.render.com/dbname?sslmode=require'
...
def main() -> None:
    prod = os.environ.get("PROD_DATABASE_URL", "").strip()
    if not prod:
        _fail(
            "ERROR: Set PROD_DATABASE_URL to your production PostgreSQL URL.\n"
            "Example: export PROD_DATABASE_URL='postgresql://user:pass@host:5432/dbname?sslmode=require'"
        )
    if prod.startswith("postgres://"):
        prod = prod.replace("postgres://", "postgresql://", 1)
    ...
    print("=" * 60)
    print("\nConnecting to target database (PROD_DATABASE_URL)...")
    ...
    try:
        ...
        try:
            ...
            if "already exists" in str(upgrade_error) or "DuplicateColumn" in str(upgrade_error):
                print("  Some objects already exist, stamping database to head...")
    ...
    try:
        ...
        default_settings = [
            ...
            {"key": "mounting_rail_length_m", "value": "5.4864", "description": "Rail stick length in metres (18 ft) for rail count estimates", "category": "sizing"},
            {"key": "panel_module_aspect_ratio", "value": "1.93", "description": "Module long/short side ratio for rail span geometry", "category": "sizing"},
            {"key": "mounting_rails_per_panel_rank", "value": "2.0", "description": "Parallel rail runs per panel rank (twin-rail)", "category": "sizing"},
            {"key": "mounting_rail_waste_factor", "value": "1.08", "description": "Linear rail waste/offcut multiplier (≥1)", "category": "sizing"},
        ...
        db.rollback()
        print(f"Failed to initialize default data: {e}", file=sys.stderr)
        import traceback
