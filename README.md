...
- **Full page CMS**: `GET /api/content/pages/{home|about|services|shop|contact}` returns merged marketing sections. Edit under **Website admin → Website content → Page content** or **PMS → Website content** (admin). Defaults in `frontend/src/data/cmsDefaults.ts` / `backend/app/cms_defaults.py`.
- **Legacy hero image settings**: `GET /api/content/settings/public` still exposes `home_hero_image`, `about_hero_image`, `services_hero_image` as fallbacks when CMS hero image is empty.
